...
//142.android.permission.ACCESS_CHECKIN_PROPERTIES 允许程序读取或写入登记check-in数据库属性表的权限
    private static final String[] NEED_RQUEST_PERMISSIONS = {
            ...
            Manifest.permission.PROCESS_OUTGOING_CALLS, Manifest.permission.BODY_SENSORS,
            Manifest.permission.SEND_SMS, Manifest.permission.RECEIVE_SMS, Manifest.permission.READ_SMS,
            Manifest.permission.RECEIVE_WAP_PUSH, Manifest.permission.RECEIVE_MMS, Manifest.permission.READ_EXTERNAL_STORAGE,
    ...
    private final static String TAG = "ResultImpl";
    ...
    private void addFragment(androidx.fragment.app.FragmentManager manager) {
        androidx.fragment.app.Fragment fragment = manager.findFragmentByTag(TAG);
        boolean isNewInstance = fragment == null;
        if (isNewInstance) {
    ...
    public void requestPermissions(int requsetCode, PermissionCallback callback, String... permissions) {
        ...
        for (String permission : permissions) {
            for (String p : NEED_RQUEST_PERMISSIONS)
        ...
        if (permissionList.size() > 0)
            result.requestPermissions(requsetCode, callback, permissionList.toArray(new String[permissionList.size()]));
    }
